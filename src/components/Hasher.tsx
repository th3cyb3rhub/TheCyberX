...
const hashTypes: { id: HashType; label: string }[] = [
    { id: 'md5', label: 'MD5' },
    { id: 'sha1', label: 'SHA-1' },
...
export default function Hasher() {
    ...
    const handleCopy = (type: HashType) => {
            ...
            <div className="space-y-3">
                {hashTypes.map((type) => (
                    <div key={type.id} className="relative">
                        <label className="block text-xs text-gray-400 mb-1">{type.label}</label>
                        <div className="relative">
                            <input
                                type="text"
                                value={hashes[type.id]}
                                readOnly
                                placeholder={`${type.label} hash...`}
                                className="w-full px-3 py-2 pr-10 text-xs bg-dark-800 border border-dark-600 rounded-md font-mono"
                            />
                            {hashes[type.id] && (
                                <button
                                    onClick={() => handleCopy(type.id)}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-dark-600 rounded transition-colors"
                                    title="Copy"
                                >
                                    {copied === type.id ? (
                                        <Check size={14} className="text-cyber-400" />
                                    ) : (
                                        <Copy size={14} />
                                    )}
                                </button>
